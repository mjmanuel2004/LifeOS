...
import { ArrowRight, Eye, EyeOff, Sparkles, Mail, Lock } from 'lucide-react';
...
export default function Login() {
    ...
    const { login, googleLoginContext, appleLoginContext } = useAuth();
    ...
    const doGoogleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            if (result.success && result.requires2FA) {
                ...
                setError(result.error || 'Erreur lors de la connexion via Google');
                setIsLoading(false);
            }
        },
    ...
    const doAppleLogin = async (response) => {
        if (response.error) {
            setError('Erreur lors de la connexion via Apple');
            return;
        }
        ...
        if (result.success && result.requires2FA) {
            ...
            setError(result.error || 'Erreur lors de la connexion via Apple');
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-[100dvh] w-full bg-[#0B0C10] flex font-sans overflow-x-hidden md:p-4 lg:p-6 lg:items-center lg:justify-center">
            ...
            <div className="w-full relative flex flex-col min-h-[100dvh] lg:hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-[#1c1c36] via-[#0B0C10] to-[#0B0C10] pointer-events-none md:rounded-[40px] md:max-w-[420px] md:m-auto md:h-[90vh] md:max-h-[850px] md:shadow-2xl md:border md:border-white/5">
                    <div className="absolute top-[8%] left-[20%] w-[2px] h-[2px] bg-white/40 rounded-full" />
                    <div className="absolute top-[18%] left-[60%] w-[3px] h-[3px] bg-blue-400/30 rounded-full blur-[1px]" />
                    <div className="absolute top-[12%] right-[15%] w-[1.5px] h-[1.5px] bg-white/60 rounded-full" />
                    <div className="absolute top-[25%] right-[25%] w-[2px] h-[2px] bg-indigo-300/40 rounded-full blur-[1px]" />
                </div>

                <div className="w-full max-w-[420px] mx-auto flex flex-col min-h-[100dvh] md:min-h-[90vh] md:max-h-[850px] relative z-10 p-6 pt-16 pb-12">
                    {/* Top Logo & Greeting */}
                    <div className="w-full flex flex-col items-center mb-10">
                        ...
                        <div className="flex items-center gap-1.5 text-[#8890A6] text-xs font-medium">
                            ...
                            <span>Bon après-midi</span>
                        </div>
                    </div>

                    {/* Title and White Curve */}
                    <div className="relative w-full mb-10">
                        <h1 className="text-[2.5rem] font-bold text-white tracking-tight leading-none">Connexion</h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 relative z-20">
                        ...
                        <div className="flex items-center justify-between mt-6">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                <div className="relative w-4 h-4 flex items-center justify-center">
                                    ...
                                    <div className="w-4 h-4 border border-[#3A435C] rounded bg-transparent peer-checked:bg-[#5282FF] peer-checked:border-[#5282FF] transition-all" />
                                    <svg className="absolute w-2.5 h-2.5 text-white opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                                        <polyline points="20 6 9 17 4 12"></polyline>
                                    </svg>
                                </div>
                                <span className="text-sm font-medium text-[#646A80] group-hover:text-[#8890A6] transition-colors">Se souvenir de moi</span>
                            </label>
                            <button
                                type="button"
                                onClick={() => navigate('/forgot-password')}
                                className="text-sm font-medium text-[#5282FF] hover:text-[#7178F2] transition-colors"
                            >
                                Mot de passe oublié ?
                            </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-10">
                            <h2 className="text-[1.75rem] font-bold text-white tracking-tight">Se connecter</h2>
                            <button
                                type="submit"
                                disabled={isLoading}
                        ...
                        <div className="flex items-center justify-between mt-auto">
                            <button
                                ...
                                className="px-5 py-2.5 rounded-full bg-[#1E2943] text-[#A5C0FF] text-[13px] font-semibold hover:bg-[#2A3755] transition-colors"
                            >
                                Créer un compte
                            </button>
            ...
            <div className="hidden lg:flex w-full max-w-[1400px] min-h-[820px] lg:my-8 bg-[#0A0D18] rounded-[40px] shadow-2xl overflow-hidden relative border border-white/5">
                ...
                <div className="w-[45%] flex flex-col justify-between px-16 py-12 relative z-10 shrink-0">
                    ...
                    {/* Main Form Area */}
                    <div className="w-full max-w-[420px] mx-auto flex flex-col pt-12">
                        {/* Greeting Chip */}
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1c1c36]/50 border border-white/5 w-max mb-8 backdrop-blur-sm">
                            <Sparkles size={14} className="text-[#8890A6]" />
                            <span className="text-[#8890A6] text-xs font-medium">Bon après-midi</span>
                        </div>

                        {/* Title Split */}
                        <div className="mb-10">
                            <h1 className="text-5xl font-extrabold tracking-tight leading-[1.15]">
                                <span className="text-white block">Connectez-vous</span>
                                <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#4FACFE] to-[#00F2FE] block mt-1">
                                    à votre espace
                                </span>
                            </h1>
                            <p className="text-[#8890A6] mt-4 font-medium text-sm">
                                Saisissez vos identifiants pour accéder à LifeOS
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                            ...
                            <div className="flex items-center justify-between mt-1 px-1">
                                <label className="flex items-center gap-3 cursor-pointer group">
                                    <div className="relative w-4 h-4 flex items-center justify-center">
                                        <input type="checkbox" className="peer sr-only" />
                                        <div className="w-4 h-4 border border-[#3A435C] rounded-[4px] bg-[#121526] peer-checked:bg-[#5282FF] peer-checked:border-[#5282FF] transition-all" />
                                        <svg className="absolute w-2.5 h-2.5 text-white opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                                            <polyline points="20 6 9 17 4 12"></polyline>
                                        </svg>
                                    </div>
                                    <span className="text-xs font-semibold tracking-wide text-[#646A80] group-hover:text-[#8890A6] transition-colors">
                                        Se souvenir
                                    </span>
                                </label>
                                <button
                                    type="button"
                                    onClick={() => navigate('/forgot-password')}
                                    className="text-xs font-semibold text-[#5282FF] hover:text-[#7A9FFF] transition-colors focus:outline-none"
                                >
                                    Mot de passe oublié ?
                                </button>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-[180px] mt-4 py-3.5 rounded-2xl bg-gradient-to-r from-[#5966DB] to-[#2D82D6] flex items-center justify-center gap-2 active:scale-95 transition-transform disabled:opacity-70 disabled:active:scale-100 shadow-[0_4px_20px_-4px_rgba(45,130,214,0.4)]"
                            >
                                {isLoading ? (
                                    <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                ) : (
                                    <>
                                        <span className="text-white font-semibold text-[15px]">Connexion</span>
                        ...
                        <div className="mt-10 text-center">
                            <span className="text-[#646A80] text-[13px] font-medium">Pas encore de compte ? </span>
                            <button
                                type="button"
                                onClick={() => navigate('/register')}
                                className="text-[#5282FF] hover:text-[#7A9FFF] text-[13px] font-semibold transition-colors ml-1"
                            >
                                Créer un compte
                            </button>
                        </div>
                ...
                <div className="absolute right-0 top-0 bottom-0 w-[60%] overflow-hidden pointer-events-none z-0" style={{ clipPath: 'polygon(20% 0, 100% 0, 100% 100%, 0% 100%)' }}>
                    ...
                    <div className="absolute top-[15%] right-[15%] w-24 h-24 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] transform rotate-12 z-20 shadow-[0_8px_32px_rgba(0,0,0,0.3)] animate-pulse" style={{ animationDuration: '8s' }} />
                    <div className="absolute bottom-[20%] left-[25%] w-16 h-16 bg-blue-500/10 backdrop-blur-md border border-white/10 rounded-2xl transform -rotate-6 z-20 shadow-[0_8px_32px_rgba(0,0,0,0.3)] animate-pulse" style={{ animationDuration: '6s' }} />
                </div>
            </div>
        </div>
    );
